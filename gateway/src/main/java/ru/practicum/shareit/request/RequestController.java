...
public class RequestController {
    ...
    @PostMapping
    public ResponseEntity addItemRequest(@RequestHeader("X-Sharer-User-Id") Long userId,
                                         @RequestBody @Valid RequestDto itemRequestDto) {
        log.info("POST '/requests'. Запрос пользователя с id {} ", userId);
        ResponseEntity response = requestClient.addItemRequest(itemRequestDto, userId);
    ...
    public ResponseEntity getItemRequest(@RequestHeader("X-Sharer-User-Id") Long userId,
        ...
        ResponseEntity response = requestClient.getItemRequest(requestId, userId);
        log.info("GET '/requests/{requestId}'. Ответ, запрос: {} ", response);
        return response;
    }

    @GetMapping
    public ResponseEntity getRequestorRequest(@RequestHeader("X-Sharer-User-Id") long requestId) {
        log.info("GET '/requests'. Запрос на получение запросов с id {} ", requestId);
        ResponseEntity response = requestClient.getRequestorRequest(requestId);
        log.info("GET '/requests'. Ответ, запрос: {} ", response);
        return response;
    }

    @GetMapping("/all")
    public ResponseEntity getItemRequests(@RequestHeader("X-Sharer-User-Id") long userId,
        ...
        ResponseEntity response = requestClient.getItemRequests(userId, from, size);
        log.info("GET '/requests'. Ответ, запрос: {} ", response);
        return response;
    }
