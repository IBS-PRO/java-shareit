...
public class BookingController {
    ...
    public ResponseEntity addBooking(@RequestHeader("X-Sharer-User-Id") Long userId,
        ...
        ResponseEntity response = bookingClient.addBooking(bookingDtoRequest, userId);
        log.info("POST '/bookings'. Ответ, новая вещь: {}, успешно добавлена ", response);
        return response;
    }

    @PatchMapping("/{bookingId}")
    public ResponseEntity addBookingResolution(@RequestHeader("X-Sharer-User-Id") Long ownerId,
        ...
        ResponseEntity response = bookingClient.addBookingResolution(bookingId, ownerId, approved);
        log.info("PATCH '/bookings/{bookingId}'. Ответ, подтверждение бронирования владельцем с id {} и вещи с id {} ", ownerId, bookingId);
        return response;
    }

    @GetMapping("/{bookingId}")
    public ResponseEntity getBooking(@RequestHeader("X-Sharer-User-Id") Long userId, @PathVariable Long bookingId) {
        log.info("GET '/bookings/{bookingId}'. Запрос на получение бронирования с id {} и пользователя с id: {} ",
                bookingId, userId);
        ResponseEntity response = bookingClient.getBooking(userId, bookingId);
        log.info("GET '/bookings/{bookingId}'. Ответ, пользователь с id: {} и его бронирование {}", userId, response);
        return response;
    }

    @GetMapping("/owner")
    public ResponseEntity getOwnerBookings(@RequestHeader("X-Sharer-User-Id") long ownerId,
        ...
        ResponseEntity response = bookingClient.getOwnerBookings(ownerId, state, from, size);
        log.info("GET '/bookings/owner'. Ответ, пользователь с id {}", ownerId);
        return response;
    }

    @GetMapping
    public ResponseEntity getBookerBookings(@RequestHeader("X-Sharer-User-Id") long bookerId,
                                                      @RequestParam(name = "state", defaultValue = "ALL") String state,
        ...
        ResponseEntity response = bookingClient.getBookerBookings(bookerId, state, from, size);
